...
/* ΓöÇΓöÇ Fixed light-mode design tokens (shadcn-aligned) ΓöÇΓöÇ */
const T = {
...
/* ΓöÇΓöÇ MOCK DATA REMOVED ΓöÇΓöÇ */
...
/* ΓöÇΓöÇ Leave table (shared) ΓöÇΓöÇ */
                  ...
                  <td colSpan={9} style={{padding:'10px 16px'}}>
                    <div style={{display:'flex',gap:8,alignItems:'center'}}>
                      <input value={rejectReason} onChange={e=>setRejectReason(e.target.value)}
                        placeholder="Enter rejection reasonΓÇª"
...
/* ΓöÇΓöÇ EMPLOYEE VIEW ΓöÇΓöÇ */
function EmployeeView({onNew,onView,leaves,setLeaves}){
  const [filter,setFilter]=useState('All')
  const filters=['All','Pending','Approved','Rejected']
  // /leaves/me already returns only this user's leaves — no name filter needed
...
/* ΓöÇΓöÇ ADMIN VIEW ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
        ...
        <>
          ...
          <div style={{display:'flex',gap:10,marginBottom:14,flexWrap:'wrap',alignItems:'center'}}>
            <div style={{position:'relative'}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search employeeΓÇª"
...
/* ΓöÇΓöÇ REQUEST MODAL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.45)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20,backdropFilter:'blur(2px)'}}>
      <div style={{background:'#fff',borderRadius:14,width:'100%',maxWidth:520,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 8px 32px rgba(15,23,42,0.18)',border:`1px solid ${T.border}`}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 22px',borderBottom:`1px solid ${T.border}`}}>
          ...
          <button onClick={onClose} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:8,padding:'5px 12px',fontWeight:600,cursor:'pointer',color:T.muted}}>Γ£ò</button>
        </div>
        <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
          {isAdmin&&(
            <div>
              <label style={{display:'block',fontSize:12,fontWeight:600,color:T.muted,marginBottom:5}}>Employee</label>
              <select style={IF}><option value="">Select employeeΓÇª</option>
          ...
          <div>
            ...
            <textarea value={note} onChange={e=>setNote(e.target.value)} rows={3} placeholder="Add a reason for your leave requestΓÇª"
              style={{...IF,resize:'vertical'}}/>
          </div>

          {/* ΓöÇΓöÇ File upload (shown for Sick Leave) ΓöÇΓöÇ */}
          {ltype==='Sick Leave'&&(
            <div>
              <label style={{display:'block',fontSize:12,fontWeight:600,color:T.muted,marginBottom:5}}>
                Attachment <span style={{color:RD}}>*</span>{' '}
                <span style={{color:T.muted,fontWeight:400}}>(Required ΓÇö PDF, JPG, PNG)</span>
              </label>
...
/* ΓöÇΓöÇ ALLOCATION MODAL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{background:'#fff',borderRadius:16,width:'100%',maxWidth:480,boxShadow:'0 20px 60px rgba(0,0,0,0.18)'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 22px',borderBottom:'1px solid #f3f4f6'}}>
          ...
          <button onClick={onClose} style={{background:'#f3f4f6',border:'none',borderRadius:8,padding:'5px 12px',fontWeight:700,cursor:'pointer'}}>Γ£ò</button>
        </div>
        <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
          <div>
          ...
          <div>
            ...
            <textarea rows={2} placeholder="Optional noteΓÇª" style={{...IF,resize:'vertical'}}/>
          </div>
        </div>
...
/* ΓöÇΓöÇ DETAIL MODAL ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function DetailModal({leave,onClose,canApprove,onApprove}){
  ...
  const rows=[
    ...
    ['Period',`${leave.start} ΓÇô ${leave.end}`],
      ...
      <div style={{background:'#fff',borderRadius:16,width:'100%',maxWidth:480,boxShadow:'0 20px 60px rgba(0,0,0,0.18)'}}>
        ...
        {canApprove&&leave.status==='Pending'&&(
          <div style={{display:'flex',gap:8,padding:'12px 22px',borderTop:'1px solid #f3f4f6'}}>
            <Btn onClick={()=>{onApprove(leave.id);onClose()}} style={{background:'#f0fdf4',color:'#16a34a',border:'1px solid #bbf7d0'}}>Γ£à Approve</Btn>
            <Btn style={{background:'#fef2f2',color:RD,border:'1px solid #fecaca'}}>Γ¥î Reject</Btn>
          </div>
        )}
...
/* ── Role helpers — derived from stored auth, not a UI switcher ── */
...
export default function TimeOff(){
  ...
  const _rm = {ADMIN:'Admin',HR_OFFICER:'HR Officer',PAYROLL_OFFICER:'Payroll Officer',EMPLOYEE:'Employee'}
  const role = _rm[_u?.role] || 'Employee'
  ...
  const handleApplyLeave = async (data) => {
    try {
      ...
      // Use real logged-in user identity
      const me = JSON.parse(localStorage.getItem('user')||'{}')
      const newLeave = {
        id: res.data.data.id,
        emp: me.name || `${me.firstName||''} ${me.lastName||''}`.trim() || 'Me',
        empId: me.loginId || '',
        type: data.ltype,
        start: data.from,
        end: data.to,
  ...
  return(
    <div style={{display:'flex',minHeight:'100vh',background:T.bg,fontFamily:'inherit',colorScheme:'light'}}>
      ...
      <div style={{flex:1,marginLeft:64,padding:'28px 28px 48px',minWidth:0}}>
        {/* Role is read from auth — no switcher exposed */}
