...
// ── Tag List (Skills / Certifications) ────────────────────────────────────────
function TagList({ items, color = 'default' }) {
...
export default function EmployeeProfile() {
  ...
  if (loading) return (
    <AppLayout title="Employee Profile" actions={backButton}>
  ...
  return (
    <AppLayout title="Employee Profile" actions={backButton}>
      <div className="max-w-4xl mx-auto space-y-4 anim-fade-up">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 rounded-xl border border-border/50 bg-card p-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-3 mt-2">
              ...
              {emp.user?.role     && <span className="flex items-center gap-1 text-xs text-muted-foreground"><User className="size-3" />{emp.user.role.replace('_', ' ')}</span>}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Section title="Work Information" icon={Briefcase}>
            ...
            {emp.manager && (
              <InfoRow icon={User} label="Manager" value={`${emp.manager.firstName} ${emp.manager.lastName}${emp.manager.designation ? ` — ${emp.manager.designation}` : ''}`} />
            )}
          </Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Section title="Skills" icon={Award}>
            <TagList items={emp.skills} />
          </Section>

          {/* ── Certifications ── */}
          <Section title="Certifications" icon={Award}>
            <TagList items={emp.certifications} />
          </Section>
        </div>

        {/* ── Bank & Payroll (Admin / Payroll Officer only) ── */}
        {showBank && (
